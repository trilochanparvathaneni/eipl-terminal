...
function MiniTooltip({
  ...
  return (
    <div className="rounded-md border border-white/15 bg-slate-900/95 px-2 py-1 shadow-lg backdrop-blur-md">
...
function Top5Card() {
  ...
  return (
    <Card className="clean-lux-card">
      <CardHeader className="flex flex-row items-start justify-between gap-2 space-y-0">
        <div>
          <CardTitle className="text-lg">Top 5 {tab === "client" ? "Clients" : "Products"}</CardTitle>
          <CardDescription className="text-slate-600">Trips & Volume · {periods[per].text}</CardDescription>
        </div>
        ...
        {data.map((d) => (
          <div key={`${tab}-${per}-${d.r}`}>
            <div className="flex items-baseline justify-between mb-1.5">
              <span className="mr-2 truncate text-sm font-semibold text-slate-800">{d.r}. {d.n}</span>
              <span className="whitespace-nowrap text-sm font-bold text-slate-600">{d.trips} trips</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500">{d.volume.toLocaleString()} KL</span>
  ...
  return (
    <Card className="clean-lux-card">
      <CardHeader>
        ...
        <CardDescription className="text-slate-600">Key metrics · {periods[per].text}</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-2 gap-3 mb-4">
          {[Truck, Clock, RefreshCw, PackageCheck].map((Icon, i) => (
            <div key={kpis[i].label} className="rounded-xl border border-slate-200 bg-slate-50 p-3.5">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[11.5px] font-semibold text-slate-500">{kpis[i].label}</span>
                <Icon className="h-4 w-4 text-slate-500" />
              </div>
              <div className="text-xl font-bold text-slate-800">{kpis[i].value}</div>
              <span className={`text-[11px] font-semibold ${kpis[i].up ? "text-emerald-600" : "text-amber-600"}`}>{kpis[i].delta} vs prev</span>
            </div>
          ))}
        </div>
...
const movementStages = [
  { stage: "Outside Queue",  count: 8,  Icon: Clock,        bg: "bg-slate-50", border: "border-slate-200", text: "text-amber-700", desc: "Waiting at gate"       },
  { stage: "Weighbridge",    count: 2,  Icon: Scale,        bg: "bg-slate-50", border: "border-slate-200", text: "text-sky-700",   desc: "Weigh-in (2 bridges)"  },
  { stage: "Safety Check",   count: 3,  Icon: ShieldCheck,  bg: "bg-slate-50", border: "border-slate-200", text: "text-violet-700",desc: "HSE inspection"        },
  { stage: "Waiting Bay",    count: 4,  Icon: Hourglass,    bg: "bg-slate-50", border: "border-slate-200", text: "text-amber-700", desc: "In yard, no bay yet"   },
  { stage: "At Bay",         count: 12, Icon: PackageCheck, bg: "bg-slate-50", border: "border-slate-200", text: "text-emerald-700", desc: "Loading in progress"   },
  { stage: "Doc Clearance",  count: 3,  Icon: FileText,     bg: "bg-slate-50", border: "border-slate-200", text: "text-sky-700",   desc: "Weigh-out & docs"      },
  { stage: "Exiting",        count: 2,  Icon: LogOut,       bg: "bg-slate-50", border: "border-slate-200", text: "text-slate-700", desc: "Seal & exit gate"      },
]
          ...
          {movementStages.map(({ stage, count, Icon, bg, border, text, desc }, i) => (
            <div key={stage} className="flex items-center gap-1 shrink-0">
              <div className={`rounded-xl border ${bg} ${border} px-3 py-3 flex flex-col items-center gap-1.5 min-w-[88px]`}>
                ...
                <span className="text-center text-[11px] font-semibold leading-tight text-slate-900">{stage}</span>
                <span className="text-center text-[10px] leading-tight text-slate-500">{desc}</span>
              </div>
      ...
      <CardContent>
        <div className="flex gap-3 mb-4">
          {stats.map(({ l, v, c, Icon }) => (
            <div key={l} className="flex-1 rounded-lg border border-white/10 bg-white/[0.04] py-2.5 px-3 text-center">
              <Icon className="h-3.5 w-3.5 mx-auto mb-1" style={{ color: c }} />
              <div className="text-xl font-bold" style={{ color: c }}>{v}</div>
              <div className="mt-0.5 text-[10.5px] font-semibold text-slate-400">{l}</div>
            </div>
          ))}
        </div>
        <div className="h-[100px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={hourlyGate} barGap={2}>
              ...
              <YAxis hide />
              <Tooltip
                contentStyle={{ fontSize: 11, borderRadius: 6, border: "1px solid rgba(255,255,255,0.15)", background: "rgba(15,23,42,0.95)", color: "#f8fafc" }}
                itemStyle={{ padding: 0 }}
...
export default function DashboardPage() {
      ...
      <CardContent className="space-y-3">
        <div className="grid grid-cols-3 gap-2">
            <div className="rounded-lg border border-slate-200 bg-white p-2.5">
              <p className="text-[11px] font-semibold text-slate-400">Recent Bookings</p>
            <p className="text-xl font-bold text-slate-900">{recentBookings.length}</p>
          </div>
  ...
  return (
    <PeriodContext.Provider value={{ per }}>
      <div className="clean-lux-shell -mx-4 space-y-6 rounded-2xl p-4 lg:-mx-6 lg:p-6">
        <div className="flex items-center gap-2 rounded-lg border border-amber-300 bg-amber-100 px-4 py-2.5 text-sm text-amber-900">
        ...
        <DashboardHeader
          ...
          controls={(
            <FilterBar
              sticky={false}
              className="border border-slate-200 bg-white p-3 shadow-[0_4px_20px_rgba(0,0,0,0.03)]"
              right={(
              ...
              {recentBookings.map((b) => (
                <tr key={b.id} className="border-b border-slate-100 last:border-0">
                  <td>
                    ...
                    <div className="text-xs text-slate-400 sm:hidden">{b.client}</div>
                  </td>
                  ...
                  <td className="hidden sm:table-cell text-slate-400">{b.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
